...
The build report system generates comprehensive reports on code coverage, security vulnerabilities, and performance metrics. Reports are automatically generated on pushes to the main branch and deployed to GitHub Pages.

### Components

#### Coverage Reports (`scripts/coverage/`)

- **collect-coverage.sh**: Collects code coverage using c8
  - Detects if JavaScript unit tests exist (*.test.js or *.spec.js)
  - If no unit tests found, creates informational report
  - If unit tests exist, runs them with c8 coverage
  - Outputs: `build-reports/coverage/data.json`

- **generate-coverage-report.sh**: Generates HTML coverage report
  - Parses c8 coverage data
  - Creates detailed HTML report with file-by-file breakdown
  - Links to GitHub source files
  - Outputs: `build-reports/coverage/index.html`

**Note**: Currently no JavaScript unit tests exist. The test suite consists of integration tests that test the deployed service. Coverage reports will show "No unit tests found" until unit tests are added.

#### Security Reports (`scripts/security/`)

- **run-security-scan.sh**: Runs security vulnerability scans
  - Executes `npm audit` for dependency vulnerabilities
  - Runs ESLint with security plugins
  - Outputs: `build-reports/security/{npm-audit.json,eslint-security.json,data.json}`

- **generate-security-report.sh**: Generates HTML security report
  - Parses npm audit and ESLint results
  - Creates summary with vulnerability counts
  - Shows detailed findings with severity levels
  - Outputs: `build-reports/security/index.html`

#### Performance Reports (`scripts/performance/`)

- **collect-performance-metrics.sh**: Collects performance metrics
  - Runs all API tests with timing measurements
  - Measures response times for each test suite
  - Calculates min, max, avg response times
  - Outputs: `build-reports/performance/data.json`

- **generate-performance-report.sh**: Generates HTML performance report
  - Shows performance metrics for each test suite
  - Highlights slow tests (>2000ms)
  - Includes overall statistics
  - Outputs: `build-reports/performance/index.html`

#### Main Report (`scripts/reports/`)

- **generate-main-report.sh**: Generates main TOC page
  - Aggregates data from all three reports
  - Creates dashboard with summary cards
  - Links to detailed reports
  - Outputs: `build-reports/index.html`
...
```bash
# Run individual report generation
bash scripts/coverage/collect-coverage.sh
bash scripts/coverage/generate-coverage-report.sh
...
# For performance tests, start local server first
npm run dev:local &
sleep 5
bash scripts/performance/collect-performance-metrics.sh
bash scripts/performance/generate-performance-report.sh
...
# View reports
open build-reports/index.html  # macOS
xdg-open build-reports/index.html  # Linux
```

#### CI/CD

Reports are automatically generated by the `.github/workflows/build-report.yml` workflow on pushes to main branch. The workflow:

1. Runs coverage collection
...
View published reports at: `https://curtcox.github.io/hashbin.org/`
...
│   ├── index.html          # Coverage report
│   ├── data.json           # Coverage data (c8 format)
│   └── raw/                # Detailed c8 HTML report
├── security/
│   ├── index.html          # Security report
│   ├── data.json           # Summary data
│   ├── npm-audit.json      # npm audit output
│   └── eslint-security.json # ESLint output
└── performance/
    ├── index.html          # Performance report
    └── data.json           # Performance metrics
```
...
- All findings are **warnings only** and do not block builds
- Reports include links to GitHub source files for easy navigation
- JSON data files are in native tool formats for easy integration
- Coverage requires JavaScript unit tests to function (currently none exist)
- Performance tests require a running local server
